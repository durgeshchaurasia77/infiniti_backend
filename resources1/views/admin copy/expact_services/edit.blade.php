            ...
            <div class="col-md-12">
                ...
                <div class="page-header">
                    <div class="row align-items-center">
                        <div class="col-sm-8">
                            <h4 class="page-title">Edit Expact Services</h4>
                        </div>
                    </div>
                </div>
                <!-- /Page Header -->
                ...
                <div class="card">
                    <div class="card-body">
                        <form action="{{ route('expact-services-update') }}" method="post" class="formSubmit" enctype="multipart/form-data">
                            @csrf
                            <input type="hidden" name="id" value="{{ $expact_servicesData->id ?? '' }}">
                            <div class="modal-body">
                                <div class="first d-flex" style="gap:30px;">
                                    <div class="mb-2 w-100">
                                        <label class="col-form-label">Title<span class="text-danger">*</span></label>
                                        <input type="text" class="form-control" name="title" id="title" placeholder="Please Enter title..." value="{{ $expact_servicesData->title ?? '' }}" required>
                                    </div>
                                    <div class="mb-2 w-100">
                                        <label class="col-form-label">Sub Title<span class="text-danger">*</span></label>
                                        <input type="text" class="form-control" name="subtitle" id="subtitle" placeholder="Please Enter subtitle..." value="{{ $expact_servicesData->subtitle ?? '' }}" required>
                                    </div>
                                    <div class="mb-2 w-100">
                                        <label class="col-form-label">YouTube Link<span class="text-danger">*</span></label>
                                        <input type="url" class="form-control" name="youtube_link" id="youtube_link" placeholder="Please Enter youtube_link..." value="{{ $expact_servicesData->youtube_link ?? '' }}" required>
                                    </div>
                                </div>
                                <div id="dynamic-fields">
                                    @if(isset($expact_servicesData->details) && count($expact_servicesData->details) > 0)
                                        @foreach($expact_servicesData->details as $key => $detail)
                            ...
                            <div class="modal-footer">
                                <div class="d-flex align-items-center justify-content-end m-0">
                                    <a href="{{route('expact-services-list')}}" class="btn btn-light me-2" >Cancel</a>
