...
@section('content')
                    ...
                    <div class="container">
                        <div class="row g-5 banner-carousel-text-one">
                            <div class="col-xl-12 col-lg-12">
                                <h4 class="text-white display-4 mb-4 wow fadeInDown" data-wow-delay="0.1s">Destination Services</h4>
                                    <ol class="breadcrumb d-flex justify-content-center mb-0 wow fadeInDown" data-wow-delay="0.3s">
                                        <li class="breadcrumb-item"><a href="{{route('web_home')}}" class="text-white">Home</a></li>
                                        <li class="breadcrumb-item active text-primary">Destination Services</li>
                                    </ol>
                            </div>
                        </div>
                    </div>
        ...
        <!-- Destination Services Start -->
        <div class="container-fluid destination py-5 px-5">
            <div class="row">
                <div class="col-xl-8 col-lg-8 mb-4">
                    ...
                    <ol>
                        @foreach($destinationservices as $destinationservice)
                            <li>
                                <a href="#destination_services{{$destinationservice->id ?? ''}}">{{$destinationservice->title ?? ''}}</a>
                            </li>
                        @endforeach
                    </ol>
                    </div>
                    @foreach($destinationservices as $key=> $destinationservice)
                        <div id="destination_services{{$destinationservice->id ?? ''}}">
                            <h5>{{$key+1}} {{$destinationservice->title ?? ''}}</h5>
