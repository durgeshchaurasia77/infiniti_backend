                    ...
                    <div class="container">
                        <div class="row g-5 banner-carousel-text-one">
                            <div class="col-xl-12 col-lg-12">
                                <h4 class="text-white display-4 mb-2 wow fadeInDown" data-wow-delay="0.1s">FAQ</h4>
                ...
                <div class="row">
                    <div class="col-xl-8 col-lg-8 mb-4">
                        <div class="accordion" id="accordionExample">
                            <!-- Exit Permission -->
                            @foreach($faqDatas as $key => $faqData)
                              <div class="accordion-item">
                                <h2 class="accordion-header" id="heading{{$faqData->id ?? ''}}">
                                  <button class="accordion-button {{ $key == 0 ? '' : 'collapsed' }}" type="button" data-bs-toggle="collapse" data-bs-target="#collapse{{$faqData->id ?? ''}}" aria-expanded="true" aria-controls="collapse{{$faqData->id ?? ''}}">
                                      {{$faqData->question ?? ''}}
                                  </button>
                                </h2>
                                <div id="collapse{{$faqData->id ?? ''}}" class="accordion-collapse collapse {{ $key == 0 ? 'show' : '' }}" aria-labelledby="heading{{$faqData->id ?? ''}}" data-bs-parent="#accordionExample">
