...
@section('content')
                    ...
                    <div class="container">
                        <div class="row g-5 banner-carousel-text-one">
                            <div class="col-xl-12 col-lg-12">
                                <h4 class="text-white display-4 mb-4 wow fadeInDown" data-wow-delay="0.1s">FRRO Location</h4>
                                    <ol class="breadcrumb d-flex justify-content-center mb-0 wow fadeInDown" data-wow-delay="0.3s">
                                        <li class="breadcrumb-item"><a href="{{route('web_home')}}" class="text-white">Home</a></li>
                                        <li class="breadcrumb-item active text-primary">FRRO Location</li>
                                    </ol>
                            </div>
                        </div>
                    </div>
        ...
        <!-- FRRO Start -->
        <div class="container-fluid frro py-5 px-5">
                <div class="row">
                    <div class="col-xl-8 col-lg-8 mb-4">
                        <div class="accordion" id="accordionExample">
                            <!-- New Delhi -->
                            @foreach($frrolocation as $key=>$frrolocations)
                              <div class="accordion-item">
                                <h2 class="accordion-header" id="he{{$frrolocations->id ?? ''}}">
                                  <button class="accordion-button {{ $key == 0 ? '' : 'collapsed' }}" type="button" data-bs-toggle="collapse" data-bs-target="#frro{{$frrolocations->id ?? ''}}" aria-expanded="true" aria-controls="frro{{$frrolocations->id ?? ''}}">
                                      {{$frrolocations->title ?? ''}}
                                  </button>
                                </h2>
                                <div id="frro{{$frrolocations->id ?? ''}}" class="accordion-collapse collapse {{ $key == 0 ? 'show' : '' }}" aria-labelledby="he{{$frrolocations->id ?? ''}}" data-bs-parent="#accordionExample">
                                  <div class="accordion-body">
                                      <div class="address">
                                          {!! $frrolocations->description ?? '' !!}
                                          <p>
                                              <iframe
                                                  src="https://www.google.com/maps?q={{$frrolocations->latitude ?? '0'}},{{$frrolocations->longitude ?? '0'}}&hl=es;z=14&output=embed"
                                                  width="100%"
                                                  height="450"
                                                  frameborder="0"
                                                  style="border:0;"
